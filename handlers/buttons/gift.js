...
async function renderCardCanvas(cards) {
  ...
  const canvas = Canvas.createCanvas(cards.length * (CARD_W + GAP), CARD_H);
...
module.exports = async function giftButtonHandler(interaction) {
  ...
  if (action === 'confirm') {
    ...
    session.page = 0;
    session.resultCards = result.cards;
    await session.save();
  ...
  if (action === 'page') {
    ...
    const cards = await Card.find({ cardCode: { $in: slice.map(c => c.cardCode) } }).lean();
    const cardMap = new Map(cards.map(c => [c.cardCode, c]));
    const ordered = slice.map(s => cardMap.get(s.cardCode));
    const attachment = ordered.length > 0 && !session.auth ? await renderCardCanvas(ordered) : null;
    ...
    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId(`gift:page:${sessionId}:${page - 1}`).setLabel(' • Previous').setStyle(ButtonStyle.Secondary).setDisabled(page === 0),
      new ButtonBuilder().setCustomId(`gift:confirm:${sessionId}`).setLabel('Confirm').setStyle(ButtonStyle.Success),
      new ButtonBuilder().setCustomId(`gift:cancel:${sessionId}`).setLabel('Cancel').setStyle(ButtonStyle.Danger),
      new ButtonBuilder().setCustomId(`gift:page:${sessionId}:${page + 1}`).setLabel('Next • ').setStyle(ButtonStyle.Secondary).setDisabled((page + 1) * PAGE_SIZE >= session.cards.length)
    );
...
async function renderSummary(interaction, session, page, pingRecipient) {
  if (!session.cards || session.cards.length === 0) {
    ...
    if (pingRecipient) {
      await interaction.followUp({ content: `-# <@${session.targetId}> received currency!` });
    }
  ...
  const row = new ActionRowBuilder().addComponents(
    ...
    new ButtonBuilder().setCustomId(`gift:summary:${session.id}:${page + 1}`).setLabel('Next • ').setStyle(ButtonStyle.Secondary).setDisabled((page + 1) * PAGE_SIZE >= session.cards.length)
  );
